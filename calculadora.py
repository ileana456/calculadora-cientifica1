...
def potencia():
    try:
        valor = entrada.get() # type: ignore
        if "^" in valor:
            base, exponente = valor.split("^")
...
def crear_boton(parent, texto, fila, columna, comando, color_fondo="#8a0d6f", color_texto="black", ancho=1, alto=1):
...
if __name__ == "__main__":
    ...
    frame_pantalla = tk.Frame(ventana, bg="#000000")
    frame_pantalla.grid(row=0, column=0, columnspan=6, padx=10, pady=10, sticky="ew")

    # Label de estado (memoria)
    status_label = tk.Label(frame_pantalla, text="", bg="#0a2d4f", fg="white", font=("Arial", 10))
    ...
    crear_boton(ventana, "MC", 1, 0, memoria_limpiar, "#5C1467", "white")
    crear_boton(ventana, "MR", 1, 1, memoria_recuperar, "#3d1745", "white")
    ...
    crear_boton(ventana, "←", 1, 5, borrar, "#191816", "white")

    # Fila 2: Funciones científicas superiores
    crear_boton(ventana, "sin", 2, 0, seno, "#58205E", "white")
    crear_boton(ventana, "cos", 2, 1, coseno, "#3E0929", "white")
    crear_boton(ventana, "tan", 2, 2, tangente, "#7a1b6c", "white")
    crear_boton(ventana, "log", 2, 3, logaritmo, "#651d54", "white")
    crear_boton(ventana, "ln", 2, 4, logaritmo_natural, "#401d43", "white")
    crear_boton(ventana, "(", 2, 5, lambda: escribir("("), "#430c43", "white")
    ...
    crear_boton(ventana, "±", 6, 1, cambiar_signo, "#34495e", "white")
